...
- q：关键词。
- qf：搜索范围（`title` / `content`），缺省搜标题+正文。
- tags：标签列表。
- difficulty：难度。
- creatorId：创建人。
- visibility：可见性。
...
- title 和 content 的全文检索（`qf=title|content` 可限定单一字段，缺省两者都搜）。
- tags 精确匹配。
- difficulty 精确匹配。
- creatorId 精确匹配。
- visibility 精确匹配。
...
- 登录页。
- 题目列表页（类型筛选：回想/文章；搜索范围：全部/仅标题/仅正文）。
- 题目详情页。
- 题目编辑页（含类型选择）。
- 系列笔记页：目录树形式——系列行（整行可点击折叠/展开，默认折叠）+ 展开后显示文章目录；系列行与文章行均支持同级拖拽排序；点击文章行跳转详情页；支持按系列名称/简介/文章标题页内搜索。管理操作仅对自己创建的系列显示。
- 管理员用户管理页。
- 导入导出页。

### 8.2 全局布局与保活约定

- 固定顶部工具栏：`PageToolbar` 共享组件经 Teleport 渲染到全局头部，各页面页头与主操作固定在视口顶部，内容区独立滚动；工具栏高度由全局 CSS 变量统一，页面高度计算据此适配。
- 页面保活（KeepAlive）：列表页、系列页、每日回想等有状态页面在 `App.vue` 中保活，组件以 `defineOptions` 声明同名 name；滚动位置按路径记忆恢复。
- 脏标记刷新：跨页写操作（如编辑保存）通过 `stores/dataDirty.ts` 的脏标记（`questionListDirty`、`quizDirty` 等）通知来源页；被保活页面 `onActivated` 消费标记后静默拉取最新数据并原位更新，不重置用户现场（如回想页已作答内容、展开的参考详情）。

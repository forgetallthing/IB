...
- **笔记管理**：Vditor Markdown 编辑器（标题与正文，均支持粘贴/拖拽/选择图片上传）、全站 Markdown 渲染、列表默认折叠答案、多条件勾选筛选（标签 / 难度 / 可见性 / 类型，均支持多选且本地记忆）、关键词搜索范围可选（全部 / 仅标题 / 仅正文）
- **固定顶部工具栏**：各页面页头与主操作按钮固定在视口顶部，内容区独立滚动，长列表浏览时操作始终可达
- **笔记类型与系列**：笔记分「回想（qa）/ 文章（article）」两种类型，文章不进每日回想抽题池（读与记分离）；「系列笔记」页可创建系列、把自己的文章按序组织成专栏（拖拽排序、添加/移出文章、改名、删除），支持按系列名称/简介/文章标题页内搜索，他人系列按可见性只读浏览
- **每日回想**：间隔重复学习系统——按出现次数加权随机抽题（越陌生越优先，仅回想类型笔记），对照回忆后自评反馈（没记住清零重推 / 模糊 +1 / 记住了 +2 / 完全掌握不再推送）；参考详情面板内置「编辑」入口，编辑保存后返回时原题与作答内容保持不变、标题答案静默刷新；点击自评选项才计一次完整回想；支持手动档位（1-4 档重置出现次数并恢复自动调节，0 档完全掌握锁定）
- **语音听写**：小程序「录音」tab 基于微信同声传译插件实时转写语音，文字流式同步到同账号 Web 端「每日回想」作答框（在已有内容后追加写入；检测到手动编辑时中间结果不覆盖，整句确认后追加到末尾）
...
- **权限分层**：用户管理 / 标签管理 / 数据备份仅管理员可见；个人工作台与数据看板所有用户可用
- **多端**：Web（响应式）+ 微信小程序（Taro），数据互通
- **PWA 主屏图标**：iPad/安卓 Pad/手机可将网站「添加到主屏幕」，桌面图标点击直达，独立窗口运行
- **数据备份**：管理员可在「系统设置 → 数据备份」一键导出整个 MongoDB 数据库（mongodump 归档 `.archive.gz`，含图片），可用 `mongorestore` 恢复
...
Taro 4 + React + TypeScript 微信小程序端。功能：笔记列表（点击进详情）、笔记详情/编辑（view/edit 双模式，编辑权限仅创建者与管理员可见按钮）、「新建」tab（常驻编辑器）、「录音」tab（语音实时转文字并同步 Web 端每日回想）、登录、我的。
...
- 注意不要同时跑两个 watch / 与 `build:weapp` 产物混写 dist，同样会白屏

### 录音转文字（微信同声传译插件）

「录音」tab 基于微信同声传译插件实时转写语音，转写文字经服务器 `/api/voice/stream` 中转（SSE），实时写入同账号 Web 端「每日回想」的作答框。

- 使用前在小程序后台（mp.weixin.qq.com → 设置 → 第三方设置 → 插件管理）添加「微信同声传译」0.3.10（`app.config.ts` 的 `plugins.WechatSI`，provider `wx069ba97219f66d99`）
- **真机使用必须在后台「用户隐私保护指引」声明「麦克风」权限**，否则录音授权静默失败（开发者工具模拟器看不出来）；首次录音依次弹隐私授权与麦克风授权
- 插件回调为属性赋值式：`manager.onRecognize = fn`（中间结果）、`manager.onStop = fn`（整句结果在 `res.result`）；没有 `onResult` 回调；`requirePlugin` 直接用运行时注入的裸标识符（Taro 的 `Taro.requirePlugin` 无运行时实现）
- 插件单段录音最长 60s，`onStop` 后自动续录保持会话连续（上限 20 段）；开始/结束录音会向服务器发 start/stop 会话事件，Web 端凭此显示「手机语音输入中」并以作答框现有内容为基底追加写入
- 识别异常优先用开发者工具「真机调试」看 Console 里 `[Voice]` 前缀日志；模拟器对插件识别支持有限，最终以真机为准
